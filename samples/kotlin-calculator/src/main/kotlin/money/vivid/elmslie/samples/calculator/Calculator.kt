...
class Calculator {
  ...
  fun errors(): Flow<Effect.NotifyError> =
    store.effects.filter { it is Effect.NotifyError }.map { it as Effect.NotifyError }

  fun results(): Flow<Effect.NotifyNewResult> =
    store.effects.filter { it is Effect.NotifyNewResult }.map { it as Effect.NotifyNewResult }
}
